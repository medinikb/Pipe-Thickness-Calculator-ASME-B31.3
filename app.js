const fallbackPipeLibrary = {
    ...
    function getInputs() {
      ...
      const tolPercent = toNumber(els.tolerance, "Mill tolerance");
      ...
      checkRange(tolPercent, 0, 25, "Mill tolerance", "%", "tolerance");
      ...
      if (materialSpecCheck && !materialSpecCheck.within) {
        throw validationError(materialSpecCheck.message, "temperature");
      }
    ...
    function calculate(options = {}) {
      try {
        ...
        const tm = t + input.c;
        const requiredNominal = tm / (1 - input.tolPercent / 100);
    ...
    function renderFlowPanel(result) {
      ...
      els.flowTolerancePct.textContent = round(result.input.tolPercent, 2);
    ...
    function renderError(message) {
      ...
      clearResultValues();
      els.warningsBox.innerHTML = buildWarningAssessmentTable([{ level: "bad", text: message }]);
    }
              ...
              <div class="flow-table-wrap">
                <table class="flow-table report-flow-table">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td><span>3</span></td>
                      <td>Apply mill tolerance (${round(result.input.tolPercent, 2)}%)</td>
                      <td>tn = tm / (1 - tolerance)</td>
          ...
          <section class="report-section report-design-input-section">
            ...
            <table class="report-table two-column">
              <tbody>
                ...
                ${reportRow("Mill tolerance", `${round(i.tolPercent, 2)}%`)}
              </tbody>
            </table>
          </section>
...
`Mill tolerance: ${round(i.tolPercent, 3)} %\n\n` +
`2. FORMULA USED\n` +
`t = P × D / [2 × (S × E × W + P × Y)]\n` +
`tm = t + c\n` +
`Required nominal thickness = tm / (1 - mill tolerance)\n\n` +
`3. CALCULATION\n` +
...
`Required nominal thickness = ${round(result.tm, 3)} / (1 - ${round(i.tolPercent, 3)} / 100) = ${round(result.requiredNominal, 3)} mm\n\n` +
...
`This tool is for preliminary engineering calculation support only. Final design shall be verified against the controlled code edition, project PMS, IBR requirements where applicable, and approved engineering judgment.`;
    ...
    function resetFlowPanel() {
      ...
      els.flowTolerancePct.textContent = round(Number(els.tolerance.value) || 0, 2);
    ...
    function exportCsv() {
      ...
      const rows = [
        ...
        ["Mill tolerance", round(r.input.tolPercent, 3), "%"],
    ...
    function currentScenarioInput() {
      ...
      return {
        ...
        E: Number(s.e.value),
        tolerance: Number(s.tolerance.value),
        D: Number(s.od.value),
        S: mat.stress,
    ...
    function calculateScenario(input) {
      ...
      const required = tm / (1 - input.tolerance / 100);
    ...
    function scenarioPresets() {
      ...
      return {
        ...
        a: { pressure: Math.min(100, basePressure * 1.22), temp: baseTemp, corrosion: baseCorrosion, E: 0.85, tolerance: baseTolerance, od: baseOd, material: findMaterialNameByKeywords(["A106", "GR B"]), pipeType: "welded" },
        ...
        c: { pressure: basePressure, temp: baseTemp, corrosion: Math.min(6, baseCorrosion + 0.5), E: baseE, tolerance: baseTolerance, od: baseOd, material: findMaterialNameByKeywords(["A312", "TP316"]), pipeType: "seamless" }
      };
    }
    ...
    function syncScenarioBaseFromResult(result) {
      ...
      s.tolerance.value = round(result.input.tolPercent, 1);
    ...
    function renderScenarioPlayground() {
      ...
      s.toleranceOut.textContent = round(active.tolerance, 2);
